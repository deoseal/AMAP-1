...
for($i=0; $i<$sample_num; $i++) {
  ...
  while ($line = <IN>) {
    ...
    if ($element[10] < 4 || $element[10] > 1001) {
      $filter = 'filtered';
    } else {
      $filter = 'not_filtered';
    }
    if ($element[10] > 0) {
      $ratio = $element[9]/$element[10];
    } else {
      $ratio = 'NA';
    }
    if ($ratio < 0.1) {
    ...
    $sample_inf[$i]{$key} = "$element[8]\t$element[9]\t$element[10]\t$ratio\t$filter";
...
for($i=0; $i<$sample_num; $i++) {
  print STDOUT "Score\tRead_num\tDepth\tRatio\tFilter\t";
}
print STDOUT "\n";
...
### SUB : help
sub help {
  print STDERR "USAGE: mergePindelList.pl pindel.list1 ... \n";
  exit(-1);
}
