...
for($i=0; $i<$sample_num; $i++) {
  ...
  while ($line = <IN>) {
    ...
    if ($element[13] < 4 || $element[13] > 1001) {
      $filter = 'filtered';
    } else {
      $filter = 'not_filtered';
    }
    $sample_inf[$i]{$key} = "$element[8]\t$element[9]\t$element[10]\t$element[11]\t$element[12]\t$element[13]\t$filter";
...
for($i=0; $i<$sample_num; $i++) {
  print STDOUT "Normalized_RD\tp-val1\tp-val2\tcn1\tcn2\tDepth\tFilter\t";
}
print STDOUT "\n";
...
### SUB : help
sub help {
  print STDERR "USAGE: mergeCNVList.pl cnv.list1 ...\n";
  exit(-1);
}
